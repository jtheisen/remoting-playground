        ...
        private Type Type(JToken token)
        {
            ...
            if (!types.TryGetValue(typeName, out type))
            {
                ...
                if (type == null)
                {
                    ...
                    if (parsedAssemblyName.Name == "System.Private.CoreLib")
                    {
                        parsedAssemblyName = new AssemblyName("netstandard");
                    }
                    else if (parsedAssemblyName.Name == "System.Linq.Expressions" && typeName == "System.Linq.IQueryable`1")
                    {
                        parsedAssemblyName = new AssemblyName("netstandard");
                    }

                    var assembly = Assembly.Load(parsedAssemblyName);
